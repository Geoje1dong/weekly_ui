...
export default function Button(props, {theme, shape, icon, onClick, margin}){
  return(
    <>
    {props.icon ? 
      <IconButton shape={props.shape} theme={props.theme} onClick={props.onClick} margin={props.margin} size={props.size}>
        {props.icon}
        {props.children}
      </IconButton>
    :
      <TextButton shape={props.shape} theme={props.theme} onClick={props.onClick} margin={props.margin} size={props.size}>
        {props.children}
      </TextButton>
    }
    </>
...
const TextButton = styled.button`
  ...
  ${props => {
    switch(props.size){
      case 'xs' : return 'padding:4px 8px; font-size:0.6rem;'
      default : return 'padding:20px; font-size:1rem;'
    }
  }}
  margin: ${props => props.margin ? props.margin : '0px'};
  font-weight:600;
